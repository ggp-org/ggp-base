    ...
    <xsl:attribute name='style'>display:none</xsl:attribute>
		...
		<div id="board">
			<xsl:for-each select="fact[relation='cell']">
				<xsl:sort select="argument[2]" order="ascending"/>
				<xsl:sort select="argument[1]"/>
				<div id="cell">
					<!--<p style="text-align: center;">-->

					<xsl:choose>
						<xsl:when test="argument[3]='x'">
							X
						</xsl:when>
						<xsl:when test="argument[3]='o'">
							O
						</xsl:when>
					</xsl:choose>
					<!--</p>-->
		...
		<div id="coins">
			...
			<table border="1">
				...
				<tr>
					...
					<xsl:for-each select="fact[relation='coins']">
						<xsl:sort select="argument[1]" order="descending"/>
						<xsl:variable name="who" select="argument[1]"/>
						<td style="text-align:center;">
							<xsl:value-of select="argument[2]"/>
							<xsl:if test="../fact[relation='tiebreaker']/argument[1]=$who">
								*
							</xsl:if>
						</td>
