...
public final class KioskPlayRequestThread extends Thread
{
	...
	public KioskPlayRequestThread(KioskGameServer gameServer, Match match, List<Move> previousMoves, List<Move> legalMoves, Role role, String host, int port, String playerName, boolean unlimited)
	{
	...
	public void run()
	{
		try
		{
			...
			String response;
			if(unlimited) {
				response = HttpReader.readAsClient(socket);
			} else {
				response = HttpReader.readAsClient(socket, match.getPlayClock() * 1000 + 1000);
			}
